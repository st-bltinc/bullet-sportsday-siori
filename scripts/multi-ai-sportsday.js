...
// ── Step 5: ファイル保存 & ブランチ push ─────────

function getRepoInfo() {
  const remoteUrl = execSync('git remote get-url origin', { cwd: ROOT_DIR }).toString().trim();
  const match = remoteUrl.match(/github\.com[:/]([^/]+)\/([^.]+?)(\.git)?$/);
  if (!match) throw new Error(`GitHub リモートURLのパースに失敗: ${remoteUrl}`);
  return { owner: match[1], repo: match[2] };
}

async function createPullRequest(branchName, featureName, discussionLog) {
  const { owner, repo } = getRepoInfo();

  const body = [
    '## 概要',
    `AIマルチエージェント（Claude / GPT-4o / Mistral）が自動生成した新機能です。`,
    '',
    `## 機能名`,
    featureName,
    '',
    ...
    '',
    '---',
    '🤖 Generated by `scripts/multi-ai-sportsday.js`',
  ].join('\n');
  ...
  const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
    ...
    body: JSON.stringify({
      title: `feat: AIが自動生成した新機能「${featureName}」`,
      head: branchName,
      base: 'main',
      body,
    }),
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub API PR作成失敗 ${res.status}: ${err}`);
  }
  return (await res.json()).html_url;
}

// ブランチ名生成（feature/ai-generated-YYYY-MM-DDTHH-MM-SS）
function newBranchName() {
  const ts = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
  return `feature/ai-generated-${ts}`;
}
...
function saveAndCommit(allFiles, featureName) {
  ...
  try {
    execSync(`git checkout -b ${branch}`, { cwd: ROOT_DIR, stdio: 'inherit' });
    ...
    console.log(`\n${C.green}${C.bold}🚀 ブランチ "${branch}" を push しました。${C.reset}`);
    return branch;
  } catch (e) {
...
コードを読んで気になった点や、どの役割が向いていると思うかも述べてください。`;
  ...
  log.push('### 🔵 Claude の役割提案\n' + claudeProposal);
  log.push('### 🟢 GPT-4o の役割提案\n' + gptProposal);
  log.push('### 🔴 Mistral の役割提案\n' + mistralProposal);
...
5. SQLiteのテーブル定義（CREATE TABLE文）`;
  ...
  log.push(`### ${designMeta.emoji} ${roles.design}（設計提案）\n` + featureProposal);
...
NGの場合は問題点と改善案を箇条書きで記載してください。`;
    ...
    log.push(`### ${reviewMeta.emoji} ${roles.review}（レビュー ${loop}回目）\n` + review);
  ...
  const branch     = saveAndCommit(allFiles, featureName);
  const savedPaths = allFiles.map(f => f.relPath).filter(Boolean);

  let prUrl = null;
  if (branch) {
    logSys('Step 6 / GitHub API で PR を作成中...');
    try {
      prUrl = await createPullRequest(branch, featureName, log.join('\n\n---\n\n'));
      console.log(`\n${C.green}${C.bold}🔗 PR 作成完了: ${prUrl}${C.reset}`);
    } catch (e) {
      console.log(`\n${C.yellow}⚠️  PR作成エラー: ${e.message}${C.reset}`);
    }
  }
